	...
	while (pUnitNode != NULL)
	{
		...
		if (pLoopUnit != NULL)
		{
			...
			if (!kUnitTeam.isAtWar(pLoopUnit->getTeam()))
			{
				...
				if ((pUnit->getOwner() != pLoopUnit->getOwner() && pUnit->getDomainType() != DOMAIN_AIR ) || pLoopUnit->AreUnitsOfSameType(*pUnit, bPretendEmbarked) || (pLoopUnit->AreUnitsOfSameType(*pUnit, bPretendEmbarked) && !GC.getGame().isOption("GAMEOPTION_REBASE_IN_FRIENDLY_CITY")))
				// RED
				{
