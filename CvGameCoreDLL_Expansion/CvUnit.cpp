...
bool CvUnit::canEnterTerrain(const CvPlot& enterPlot, byte bMoveFlags) const
{
	...
	{
		// I really wish the guys at Firaxis would just learn De Morgan's laws!!!
		// !A and !B is the same as !(A or B) - which is this case would be a feck of a lot easier to read!!!
		//if(!(m_iCanMoveImpassableCount > 0 || canMoveAllTerrain())
		if(!(m_iCanMoveImpassableCount > 0) && !canMoveAllTerrain()) // No need to add IsHoveringUnit() here as helicopters have MoveImpassable anyway
		{
...
void CvUnit::move(CvPlot& targetPlot, bool bShow)
{
	...
	if (pOldPlot && CanEverEmbark()) {
		...
		if (IsHoveringUnit()) {
			...
			if (targetPlot.isWater() != pOldPlot->isWater()) {
	...
	{
#if defined(MOD_PATHFINDER_TERRAFIRMA)
		bool bFromWater = !pOldPlot->isTerraFirma(this);
...
bool CvUnit::PlotValid(CvPlot* pPlot) const
{
	...
	case DOMAIN_LAND:
#if defined(MOD_BUGFIX_HOVERING_PATHFINDER)
		if(pPlot->getArea() == getArea() || canMoveAllTerrain() || IsHoveringUnit())
#else
